...
import com.example.pokusaj.Adapter.MyNotificationAdapter;
import com.example.pokusaj.Adapter.MyNotificationAdapter2;
import com.example.pokusaj.Common.Common;
import com.example.pokusaj.Interface.INotificationLoadListener;
...
List<MyNotification2> firstList=new ArrayList<>();
    ...
    private void loadNotification(DocumentSnapshot lastDoc) {
        ...
        if(lastDoc==null)
        {
            notificationCollection.orderBy("serverTimestamp", Query.Direction.DESCENDING)
                ...
                {
                   List<MyNotification2> myNotifications=new ArrayList<>();
                   DocumentSnapshot finalDoc=null;
                   for(DocumentSnapshot notiSnapshot:task.getResult())
                   {
                       MyNotification2 myNotification=notiSnapshot.toObject(MyNotification2.class);
    ...
    @Override
    public void onNotificationLoadSuccess(List<MyNotification2> myNotificationList, DocumentSnapshot lastDocument) {
        if(lastDocument!=null)
        {
            ...
            {
                adapter=new MyNotificationAdapter2(this,myNotificationList);
